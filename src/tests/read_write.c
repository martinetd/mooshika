...
/**
 * \file   read_write.c
 * \brief  basic test to check that both read and write work.
 *
 * basic test to check that both read and write work.
 *
 */
#ifdef HAVE_CONFIG_H
#include "config.h"
...
/* start_cm_thread or:
		pthread_t id;
		pthread_create(&id, NULL, (void * (*)(void*))msk_accept_one, trans); */
	} else { //client
